...
\subsection{Installation in Windows of \AmeliaView\ as a Standalone Program}
...
\subsection{The \texttt{amelia} class}
\label{sec:out}
...
\item \textbf{Input Data Format} - Choose the format for your dataset.
  ...
  Transport (.XPORT). Note that when using a CSV file, Amelia assumes that
  your file has a header (that is, a row at the top of the data indicating
  the variable names). 
